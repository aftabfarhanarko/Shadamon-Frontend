...
import {
    ...
    Search, MapPin, Menu, X, Plus, Inbox, User, Globe, Clock, Eye, ArrowRight, ArrowUp
} from 'lucide-react';
        ...
        <div className="flex flex-col lg:flex-row items-start justify-center">
            ...
            <div className="hidden lg:block w-[300px] flex-none space-y-4">
                    ...
                    <div className="p-2 space-y-2">
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center justify-between group cursor-pointer" onClick={() => setExpandedCategory(expandedCategory === 'main' ? null : 'main')}>
                                <h3 className="text-sm text-slate-600">{t('all_categories')}</h3>
                                <ChevronDown className={cn("w-5 h-5 text-slate-400 group-hover:text-slate-600 transition-all", (expandedCategory === 'main' || expandedCategory !== null) && "rotate-180")} />
                            </div>

                            <div className="pl-1 space-y-1">
                                <h4 className="text-sm text-slate-700 ml-4 tracking-wider">{t('sell')}</h4>
                                        ...
                                        <div key={cat._id} className="space-y-1">
                                            ...
                                            >
                                                <div className="flex items-center gap-1 text-sm text-[#0088cc] font-medium hover:underline">
                                                    <CategoryIcon className="w-4 h-4 text-slate-800 shrink-0" />
                                                    <span className={cn(expandedCategory === cat._id && "font-bold text-slate-800")}>{cat.name}</span>
                                                    <span className="text-slate-400 font-normal ml-0.5">({ads.filter(ad => ad.category === cat.name).length.toLocaleString()})</span>
                                                </div>
                                                {cat.subcategories.length > 0 && (
                                                    <ChevronDown className={cn("w-3.5 h-3.5 text-slate-400 transition-all", expandedCategory === cat._id && "rotate-180")} />
                                                )}
                                            </div>
                                                ...
                                                <div className="pl-6 space-y-1 border-l border-slate-100 ml-2 animate-in fade-in slide-in-from-top-1 duration-200">
                                                    {cat.subcategories.map(sub => (
                                                        <div key={sub._id} className="flex items-center gap-1 text-sm text-[#0088cc] hover:underline cursor-pointer group">
                                                            ...
                                                            <span>{sub.name}</span>
                                                            <span className="text-slate-400">({ads.filter(ad => ad.subcategory === sub.name).length.toLocaleString()})</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                ...
                                <div className="pl-6 pt-2 space-y-1">
                                    <div className="text-sm font-medium text-slate-700 cursor-pointer hover:text-[#0088cc] transition-colors">{t('rent')}</div>
                                    <div className="text-sm font-medium text-slate-700 cursor-pointer hover:text-[#0088cc] transition-colors">{t('jobs')}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="">
                            <div className="flex items-center justify-between group cursor-pointer" onClick={() => setExpandedLocation(expandedLocation === 'main' ? null : 'main')}>
                                <h3 className="text-sm text-slate-600">{t('location')}</h3>
                                <ChevronDown className={cn("w-5 h-5 text-slate-400 group-hover:text-slate-600 transition-all", (expandedLocation === 'main' || expandedLocation !== null) && "rotate-180")} />
                            </div>

                            <div className="pl-1 space-y-1">
                                <div className="text-sm text-slate-700 ml-4 tracking-wider cursor-pointer hover:text-[#0088cc] transition-colors">
                                    {t('all_bangladesh')}
                                </div>
                                ...
                                {locations.map(loc => (
                                    <div key={loc._id} className="space-y-1">
                                        ...
                                        >
                                            <div className="flex items-center gap-1 text-sm text-[#0088cc] font-medium hover:underline">
                                                <span className={cn(expandedLocation === loc._id && "font-bold text-slate-800")}>{loc.name}</span>
                                                <span className="text-slate-400 font-normal ml-0.5">({ads.filter(ad => ad.location === loc.name).length.toLocaleString()})</span>
                                            </div>
                                            {loc.subLocations.length > 0 && (
                                                <ChevronDown className={cn("w-3.5 h-3.5 text-slate-400 transition-all", expandedLocation === loc._id && "rotate-180")} />
                                            )}
                                        </div>
                                            ...
                                            <div className="pl-6 space-y-1 border-l border-slate-100 ml-2 animate-in fade-in slide-in-from-top-1 duration-200">
                                                {loc.subLocations.map(sub => (
                                                    <div key={sub._id} className="flex items-center gap-1 text-sm text-[#0088cc] hover:underline cursor-pointer group">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-300 group-hover:bg-[#0088cc] transition-colors" />
                                                        <span>{sub.name}</span>
                                                        <span className="text-slate-400">({ads.filter(ad => ad.subLocation === sub.name).length.toLocaleString()})</span>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                ...
                <div className="bg-white rounded-lg p-3 space-y-2">
                    <h3 className="text-[15px] text-slate-800">{t('filters')}</h3>

                    <div className="space-y-1.5">
                        <label className="text-[11px] text-slate-500 tracking-tight">{t('sort_by')}</label>
                        <div className="relative">
                            <select className="w-full bg-white border border-slate-200 rounded px-3 py-1.5 text-xs text-slate-600 appearance-none outline-none focus:ring-1 focus:ring-slate-300">
                    ...
                    <div className="space-y-2">
                        <label className="text-[11px] text-slate-500 tracking-tight">{t('filter_by')}</label>
                        <div className="space-y-1.5">
                            <label className="flex items-center gap-2 cursor-pointer group">
                                <input type="radio" name="filter" defaultChecked className="w-4 h-4 border-slate-300 text-[#0088cc] focus:ring-0" />
                                <span className="text-[13px] text-slate-600 group-hover:text-slate-900 transition-colors">{t('all')}</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer group">
                                <input type="radio" name="filter" className="w-4 h-4 border-slate-300 text-[#0088cc] focus:ring-0" />
                                <span className="text-[13px] text-slate-600 group-hover:text-slate-900 transition-colors">{t('promoted')}</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg p-3 space-y-4">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-slate-500 font-medium">
                        <Link href="/about" className="hover:text-slate-800 transition-colors">{t('about_us')}</Link>
                        <span>•</span>
                        <Link href="/terms" className="hover:text-slate-800 transition-colors">{t('terms_and_con')}</Link>
                        <span>•</span>
                        <Link href="/privacy" className="hover:text-slate-800 transition-colors">{t('privacy_policy')}</Link>
                        <Link href="/contact" className="hover:text-slate-800 transition-colors">{t('contact_us')}</Link>
                        <span>•</span>
                        <Link href="/promote" className="hover:text-slate-800 transition-colors">Promote</Link>
                    </div>

                    <div className="space-y-2.5">
                        <p className="text-[12px] text-slate-700">{t('get_our_app')}</p>
                    ...
                    <div className="space-y-2.5 pt-2">
                        <p className="text-[12px] text-slate-700">{t('get_more')}</p>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 cursor-pointer group">
                                <div className="w-2.5 h-2.5 rounded-full bg-slate-300" />
                                <span className="text-[12px] text-slate-400 group-hover:text-slate-600">{t('newsportal')}</span>
                            </div>
                            <div className="flex items-center gap-2 cursor-pointer group">
                                <div className="w-2.5 h-2.5 rounded-full bg-slate-300" />
                                <span className="text-[12px] text-slate-400 group-hover:text-slate-600">{t('social_platform')}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="w-full lg:w-[565px] flex-none space-y-4">
                ...
                <div className="bg-white rounded-lg overflow-hidden">
                    ...
                    <div className="px-5 pt-4 flex items-center justify-between border-b border-slate-50">
                        <div className="flex items-center gap-8">
                            ...
                            >
                                <span className={cn(
                                    ...
                                    activeSelectorTab === 'category' ? "text-slate-900" : "text-slate-400 hover:text-slate-600"
                                )}>
                                    Select Categorie
                                </span>
                            ...
                            >
                                <span className={cn(
                                    "text-[15px] transition-colors",
                                    activeSelectorTab === 'location' ? "text-slate-900" : "text-slate-400 hover:text-slate-600"
                                )}>
                                    Select Location
                                </span>
                        ...
                        <div className="pb-2">
                            <span className="text-sm text-slate-400 cursor-pointer hover:text-slate-600 tracking-wider">Filter</span>
                        </div>
                    </div>
                    ...
                    <div className="p-5 pt-0 flex items-center justify-between">
                            ...
                            {activeSelectorTab === 'category' ? (
                                categories.slice(0, 5).map((cat) => (
                                    <div key={cat._id} className="flex flex-col items-center gap-2 flex-none group cursor-pointer">
                                        <div className="w-[72px] h-[72px] rounded-full border-2 border-red-500 p-1">
                                            <div className="w-full h-full rounded-full bg-red-50 overflow-hidden flex items-center justify-center">
                                                {cat.photo ? (
                                                    <img
                                                        src={`${API_BASE_URL}${cat.photo.startsWith('/') ? '' : '/'}${cat.photo}`}
                                                        alt={cat.name}
                                        ...
                                        <span className="text-[11px] font-bold text-slate-600 group-hover:text-slate-900 text-center max-w-[72px] truncate">{cat.name}</span>
                                    </div>
                                ))
                            ) : (
                                locations.slice(0, 5).map((loc) => (
                                    <div key={loc._id} className="flex flex-col items-center gap-2 flex-none group cursor-pointer">
                                        <div className="w-[72px] h-[72px] rounded-full border-2 border-blue-500 p-1">
                                            <div className="w-full h-full rounded-full bg-blue-50 overflow-hidden flex items-center justify-center">
                                                {loc.photo ? (
                                                    <img
                                                        src={`${API_BASE_URL}${loc.photo.startsWith('/') ? '' : '/'}${loc.photo}`}
                                                        alt={loc.name}
                                                        className="w-full h-full object-cover"
                                                    />
                                                ) : (
                                        ...
                                        <span className="text-[11px] font-bold text-slate-600 group-hover:text-slate-900 text-center max-w-[72px] truncate">{loc.name}</span>
                                    </div>
                                ))
                            )}
                        </div>
                        ...
                        {(activeSelectorTab === 'category' ? categories.length > 5 : locations.length > 5) && (
                            <button
                                onClick={scrollRight}
                                className="w-16 h-16 rounded-full bg-[#F4F4F4] flex items-center justify-center text-slate-900 hover:bg-slate-200 hover:scale-105 active:scale-95 transition-all flex-none"
                            >
                ...
                {loading ? (
                    <div className="text-center py-20 pb-40">
                        ...
                        <p className="text-slate-400 text-sm">{t('loading_feed')}</p>
                    </div>
                ) : ads.length === 0 ? (
                    <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm min-h-[400px] flex flex-col items-center justify-center text-slate-400">
                        {/* Empty state content */}
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mb-4">
                            <Home className="w-8 h-8 text-slate-300" />
                        </div>
                        <h3 className="text-lg font-bold text-slate-600">{t('no_ads_yet')}</h3>
                    </div>
                ) : (
                    (() => {
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <button className="flex items-center gap-1.5 text-xs text-black px-3 py-1.5 rounded-full">
                                        <div className="w-2.5 h-2.5 rounded-full border-2 border-orange-500"></div>
                                        Save Search
                                    </button>
                                </div>
                                        ...
                                        {chunk[0] && (
                                            ...
                                            >
                                                ...
                                                <div className="relative h-[240px] w-full bg-slate-100">
                                                    <img
                                                        src={`${API_BASE_URL}${chunk[0].images?.[0]?.startsWith('/') ? '' : '/'}${chunk[0].images?.[0]}`}
                                                        alt={chunk[0].headline}
                                                        className="w-full h-full object-cover"
                                                    />
                                                    {/* Corner Icon */}
                                                    <div className="absolute bottom-0 right-0 p-2">
                                                        <div className="bg-[#F6AD55] p-1.5 rounded-tl-lg rounded-br-lg">
                                        ...
                                        {chunk.length > 1 && (
                                            <div className="flex flex-col gap-3">
                                                {chunk.slice(1).map((ad) => (
                                                    <div
                                                        key={ad._id}
                                                        className="bg-white rounded-xl p-2 flex gap-3 cursor-pointer"
                                                        onClick={() => router.push(`?ad=${ad._id}`, { scroll: false })}
                                                    >
                                                        {/* Image */}
                                                        <div className="w-28 h-20 bg-slate-100 rounded-lg overflow-hidden shrink-0 relative">
                                                            <img
                                                                src={`${API_BASE_URL}${ad.images?.[0]?.startsWith('/') ? '' : '/'}${ad.images?.[0]}`}
                                                                alt={ad.headline}
                                                                className="w-full h-full object-cover"
                                                            />
                                                        </div>
                                        ...
                                        <div className="relative">
                                            ...
                                            {categoryAds.length > 2 && (
                                                <button
                                                    ...
                                                    className="absolute -right-3 top-[40%] -translate-y-1/2 w-9 h-9 bg-white shadow-md rounded-full flex items-center justify-center text-slate-600 z-20 border border-slate-100 hover:bg-slate-50 opacity-100"
                                                >
            ...
            <div className="hidden lg:block w-[50px] flex-none relative self-stretch">
                <div className="sticky top-[88vh] pl-1">
                    <button
                        onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
                        className="w-10 h-10 bg-[#FFCA28] rounded-full shadow-md flex items-center justify-center hover:bg-[#FFC107] transition-colors"
                    >
                        <ArrowUp className="w-6 h-6 text-slate-800" strokeWidth={2.5} />
                    </button>
                </div>
            </div>

            {/* Right Sidebar - Popular Seller: 230px */}
            <div className="hidden lg:block w-[230px] flex-none">
                <div className="bg-white rounded-lg overflow-hidden sticky top-24 w-full">
                    <div className="p-3 pb-1">
                        <h3 className="text-base text-slate-500">{t('popular_seller')}</h3>
                    </div>

                    <div className="space-y-3 px-3 pb-3 max-h-[700px] overflow-y-auto custom-scrollbar">
                                ...
                                <div key={user._id} className="flex gap-3">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <div className="flex items-center gap-1.5 leading-tight">
                                            ...
                                            <RiCheckboxCircleFill className="w-4 h-4 text-slate-400 shrink-0" />
                                        </div>
                                        <p className="text-[11px] text-slate-400 -mt-0.5">
                                            {7000 + (idx * 137)} {t('visited')}
                                        </p>
                                        <button className="mt-1 flex items-center justify-center gap-1 px-3 py-1 border border-slate-200 rounded-full text-[11px] font-bold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all w-fit">
