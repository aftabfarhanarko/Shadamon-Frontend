      ...
      <div className="relative bg-white w-full max-w-[565px] rounded-t-lg rounded-b-none overflow-hidden flex flex-col animate-in slide-in-from-bottom-full duration-300 shadow-2xl h-[calc(100vh-80px)] font-sans">
        ...
        >
          ...
          <div className="p-3">
            ...
            {/* 5. Contact Section */}
            <div className="bg-[#EBF0F6] rounded-xl mt-3 mb-3 p-3 border border-slate-200/60 shadow-xs">
              {!(ad.hidePhone === true || ad.hidePhone === "true") && (
                <div className="flex items-center justify-between gap-3 mb-3">
                  ...
                  >
                    <div className="w-8 h-8 rounded-full bg-[#1A202C] text-white flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                      <Phone className="w-4 h-4 fill-white" />
                    </div>
                    <div className="flex flex-col justify-center leading-none">
                      <span className="text-slate-900 text-sm font-bold leading-none tracking-wide">
                        {showPhone ? primaryPhone || "N/A" : "017 XXXXXXXX"}
                      </span>
                      <span className="text-[10px] text-slate-500 font-medium hover:text-blue-600 transition-colors mt-1">
                        {showPhone ? "Hide number" : "Click to show number"}
                      </span>
                    </div>
                  </div>

                  {/* Seller Info snippet inline if available */}
                  {(ad.user?.name || ad.user?.storeName) && (
                    <div className="flex items-center gap-1.5 text-xs text-slate-700 font-semibold truncate">
                      <span className="text-[10px] text-slate-400 font-normal">Posted By:</span>
                      <span className="truncate max-w-[120px]">{ad.user?.storeName || ad.user?.name}</span>
                      {ad.user?.mVerified && (
                        <VerifiedBadge
                          className="-mt-0.5"
              ...
              {/* Extra numbers dropdown/list when revealed */}
              {showPhone && otherContactRows.length > 0 && (
                <div className="mb-3 px-2 py-2 bg-white/80 rounded-lg border border-slate-200/80 space-y-1.5 animate-in fade-in duration-200">
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                    Other Numbers:
                  </p>
                  <div className="flex flex-col gap-1.5">
                    {otherContactRows.map((row, idx) => (
                      <div key={`${row.number}-${idx}`} className="flex items-center justify-between text-xs text-slate-800">
                        <span className="font-medium">{row.number}</span>
                        <div className="flex items-center gap-1.5">
                          {row.types.map((type, tIdx) => (
                            <button
                              key={tIdx}
                              onClick={() => {
                                if (type === "whatsapp") window.open(`https://wa.me/${toIntlPhone(row.number)}`, "_blank");
                                else window.location.href = `tel:${row.number}`;
                              }}
                              className="px-2 py-0.5 rounded bg-slate-200 text-[10px] font-bold text-slate-700 hover:bg-slate-300"
                            >
                              {type}
                            </button>
                          ))}
              ...
              {/* Action Buttons Row */}
              <div className="flex flex-row items-stretch gap-1.5 relative mb-3 overflow-x-auto no-scrollbar">
                {/* Invite Button */}
                <ActionGuardButton
                  className="flex-1 min-w-[55px] h-8 bg-[#1A202C] text-white text-[11px] font-medium px-2 rounded-lg hover:bg-black transition-colors shadow-2xs"
                ...
                {!(
                  ...
                  <ActionGuardButton
                    ...
                    className="flex-1 min-w-[55px] h-8 bg-white border border-slate-300 text-slate-800 text-[11px] font-medium px-2 rounded-lg hover:bg-slate-50 transition-colors shadow-2xs"
                  >
                    Call
                  </ActionGuardButton>
                )}
...
I have sent my CV for your review.`;
                  ...
                  className="flex-1 min-w-[85px] h-8 bg-white border border-slate-300 text-slate-800 text-[11px] font-medium px-2 rounded-lg hover:bg-slate-50 transition-colors shadow-2xs flex items-center justify-center"
                >
                ...
                <div className="shrink-0 flex items-center justify-end">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg border border-slate-300 bg-white flex items-center justify-center hover:bg-slate-50 transition-colors shadow-2xs"
                  >
                    <SquareArrowOutUpRight
              ...
              {/* Quick Action Circle Icons Row */}
              <div className="pt-2 border-t border-slate-200/60 flex items-center justify-around overflow-x-auto no-scrollbar gap-2">
                <div
                  onClick={async () => {
                    const token = Cookies.get("token");
                    if (!token) {
                    ...
                    try {
                      ...
                      const data = await res.json();
                      if (res.ok) setIsNotifying(data.isNotifying);
                    } catch (err) {
                ...
                >
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center shadow-2xs transition-colors", isNotifying ? "bg-teal-500 text-white" : "bg-white text-slate-700 hover:bg-slate-100")}>
                    <Bell className="w-4 h-4" />
                  </div>
                  <span className="text-[9px] font-semibold text-slate-600">Notify</span>
                </div>
                ...
                <div
                  ...
                  className="flex flex-col items-center gap-1 cursor-pointer group shrink-0"
                >
                  <div className="w-8 h-8 rounded-full bg-white text-slate-700 hover:bg-slate-100 flex items-center justify-center shadow-2xs transition-colors">
                    <Phone className="w-4 h-4" />
                  </div>
                  <span className="text-[9px] font-semibold text-slate-600">Call Req</span>
                </div>
                ...
                >
                  ...
                  <span className="text-[9px] font-semibold text-slate-600">Similar</span>
                </div>
                ...
                >
                  ...
                  <span className="text-[9px] font-semibold text-slate-600">Share</span>
                </div>
                ...
                <div
                  ...
                  className="flex flex-col items-center gap-1 cursor-pointer group shrink-0"
                >
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center shadow-2xs transition-colors", isFavorited ? "bg-blue-500 text-white" : "bg-white text-slate-700 hover:bg-slate-100")}>
                    <Heart className={cn("w-4 h-4", isFavorited && "fill-current")} />
                  </div>
                  <span className="text-[9px] font-semibold text-slate-600">{isFavorited ? "Saved" : "Save"}</span>
                </div>
                ...
                >
                  ...
                  <span className="text-[9px] font-semibold text-slate-600">Report</span>
                </div>
                ...
                >
                  ...
                  <span className="text-[9px] font-semibold text-slate-600">Copy link</span>
                </div>
              </div>
            </div>
                      ...
                      <div className="text-xs text-slate-600 leading-relaxed">
                        ...
                        {ad.description && ad.description.length > 120 && (
                          <button
                            ...
                            className="inline-ml-1 text-xs font-bold text-slate-900 hover:text-blue-600 hover:underline cursor-pointer"
                          >
                            {isDescriptionExpanded ? ` ${t("show_less")}` : ".. See More"}
                          </button>
                        )}
                      </div>
                    </div>
